...
void CEditListCtrl::SetEableEdit(int nRow, bool bEnable)
{
	ASSERT(nRow > 0);
	m_bAble[nRow - 1] = bEnable;
}
...
void CEditListCtrl::OnBeginlabeledit(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (m_iSubItem >= 0 && m_bAble[m_iSubItem] == true)
    {
...
{	
	...
	if (m_iSubItem >= 0 && m_bAble[m_iSubItem] == true && m_edtItemEdit.m_hWnd)	
	{
		CRect	rect;
		CRect	rcEdit;
